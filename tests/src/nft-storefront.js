...
import { deploySherpaItems, setupSherpaItemsOnAccount } from "./sherpa-items";

/*
 * Deploys SherpaItems and NFTStorefront contracts to SherpaAdmin.
...
export const deployNFTStorefront = async () => {
	...
	const addressMap = {
		...
		SherpaItems: SherpaAdmin,
	};

	return deployContractByNameWithErrorRaised({ to: SherpaAdmin, name: "NFTStorefront", addressMap });
};
...
export const setupStorefrontOnAccount = async (account) => {
	// Account shall be able to store Sherpa Items
	await setupSherpaItemsOnAccount(account);
